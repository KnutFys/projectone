# -*- coding: utf-8 -*-
"""
Created on Wed Oct  2 18:42:55 2019
...
class Problem:
    ...
    def split(self,folds=5):
        ...
        '''
            ...
            self.mse = -cross_val_score(
                    reg,self.M,self.data,cv=cv,n_jobs=iterations,
                    scoring="neg_mean_squared_error").mean()            
        
            print("Using SkLearn {}-fold CV. Iterations: {}".format(
                    folds,iterations))
        print("For {}-fold cross validation:".format(folds))
        print("Average MSE: {}".format(self.mse))    
        '''
...
#Part 2: 
def oppgave_2(o=5,level_of_noise=0):
    print("Oppgave 2")
    ...
    for i in order:
        ...
        prob.seed = 10
        #Enables use of train_test_split
        prob.tts=True
        prob.solve()
    ...
    ax1.set_ylabel("MSE")
    ax1.plot(df["Polynomial"],df["MSE training set"], color = "g",
             label="Training")
    ax1.plot(df["Polynomial"],df["MSE test set"], color = "r",label="Test")
    ...
    #--------------------------------------------------
    fig.savefig("{}Oppgave2tts2.png".format(plots_path))
    #df2.to_csv("{}oppgave2.csv".format(text_path))
    dta = list()
    for i in order:
        #Sets up teh design matrix
        X = design_matrix(i,x,y)
    ...
    ax2.set_ylabel("MSE")
    ax2.plot(df["Polynomial"],df["MSE training set"], color = "g",
             label="Training")
    ax2.plot(df["Polynomial"],df["MSE test set"], color = "r",label="Test")
    fig2.legend(bbox_to_anchor=(0.5, 0.5, 0.4, 0.2))
    fig2.savefig("{}Oppgave2Kfold.png".format(plots_path))
